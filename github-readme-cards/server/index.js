...
import { renderSkillsRow } from "./cards/skillsCard.js";
import { SKILL_GROUPS, ALL_SKILL_IDS } from "./cards/skillsList.js";
...
// Lista de tecnologias disponíveis, pro front-end montar o seletor
app.get("/api/skills-list", (req, res) => {
  res.json({ groups: SKILL_GROUPS });
});

// Fileira de ícones de tecnologia escolhidos, como uma única imagem SVG
app.get("/api/skills", (req, res) => {
  const idsParam = (req.query.ids || "").toString();
  const ids = idsParam
    .split(",")
    .map((s) => s.trim())
    .filter((s) => ALL_SKILL_IDS.includes(s));

  const svg = renderSkillsRow(ids);
